...
def load_and_prepare(csv_path: str):
    ...
    # === Target: 3-class (LONG=1, SHORT=0, SKIP=2) ===
    # SKIP = both directions hit SL (choppy market, model learns when NOT to trade)
    targets = []
    for _, row in df.iterrows():
        ...
        if long_tp and short_tp:
            ...
            targets.append(2)  # SKIP — both hit SL

    df["TARGET"] = targets
    n_long = targets.count(1)
    n_short = targets.count(0)
    n_skip = targets.count(2)
    print(f"Targets: LONG={n_long}, SHORT={n_short}, SKIP={n_skip} ({n_skip/len(targets)*100:.1f}% uncertain)")
